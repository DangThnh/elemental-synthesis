...
export default class Card extends Phaser.GameObjects.Container {
    constructor(scene, x, y, cardData, isPlayer) {
        ...
        // BIẾN LƯU TRỮ TRẠNG THÁI (Giúp fix lỗi đè layer và phình to scale)
        this.originalDepth = 0;
        this.originalScale = 1;
        this.hoverTargets = [];

        // 1. Nền thẻ bài (Graphics hỗ trợ Gradient)
        this.bg = scene.add.graphics();
        this.drawBackground();

        // 2. Icon hệ nguyên tố cho thẻ Single (Sử dụng ảnh rất đẹp)
        ...
        // 5. Icons tương tác kéo thả (+ và X) -> FIX: Đặt Depth 50 để luôn nổi lên trên nền
        ...
        // 6. Tooltip thành phần nguyên tố -> FIX: Đặt Depth > 100 để luôn đè lên các lá bài khác
        this.tipBg = scene.add.rectangle(0, -118, 132, 52, 0x1a1a1a, 0.95).setStrokeStyle(2, 0xffd700).setVisible(false).setDepth(100);
        this.elementIcon1 = scene.add.image(-20, -118, null).setVisible(false).setDepth(101);
        this.elementIcon2 = scene.add.image(20, -118, null).setVisible(false).setDepth(101);
        ...
        if (active) {
            ...
            if (!this.crackTween && this.scene) {
                this.crackTween = this.scene.tweens.add({
                    targets: this, alpha: { from: 0.82, to: 1 }, duration: 520, yoyo: true, repeat: -1, ease: 'Sine.easeInOut'
                });
            }
        } else {
    ...
    setupPlayerInteractions() {
        ...
        this.on('dragstart', () => {
            ...
            // FIX LỖI ĐÈ DRAWER: Lưu layer hiện tại và gán layer = 80 (đủ đè bài khác nhưng thua Drawer 100)
            this.originalDepth = this.depth;
            this.setDepth(80);
            
            // FIX LỖI PHÌNH TO: Lấy scale hiện tại x 1.05 thay vì gán chết số 1
            this.originalScale = this.scale;
            this.scene.tweens.add({ targets: this, scale: this.originalScale * 1.05, duration: 100 });
        });
        ...
        this.on('dragend', () => {
            ...
            // Tắt viền vàng của các lá bài bị đè lên
    ...
    checkHoverTargets() {
        ...
        if (this.scene.playerCoreCard && this.scene.getReserveSlotIndexOfCard?.(this) >= 0) {
            ...
            if (z && Phaser.Math.Distance.Between(this.x, this.y, z.x, z.y) < z.r + 28) {
                ...
                if (mergeData.valid) {
                    ...
                    this.scene.playerCoreCard.drawBackground(true); // Bật viền vàng thay vì tween strokeColor
                    this.hoverTargets.push(this.scene.playerCoreCard);
                } else {
                    // Nếu không ghép được thì hiện dấu +, vì logic thả vào Core là SWAP (đổi chỗ)
                    this.iconPlus.setVisible(true);
                }
                return;
            }
        ...
        for (let target of reserveList) {
            if (target !== this && Phaser.Math.Distance.Between(this.x, this.y, target.x, target.y) < 60) {
                ...
                if (mergeData.valid) {
                    this.iconPlus.setVisible(true);
                    target.drawBackground(true); // Bật viền vàng báo hiệu có thể ghép
